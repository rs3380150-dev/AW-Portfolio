...
export const MusicDetail = () => {
  ...
  const heroImage = track.heroImage || track.cover;
  const artistName = track.artistName || "Achyut Wadhwa";
  const availableServices = services.filter((service) => track.links?.[service.key]);
  const showStorySection = track.showReleaseNote !== false || track.showListenPanel !== false;
  ...
  return (
    <div className="music-detail-page bg-[#f2f0ea] text-[#050505]" data-testid={`music-detail-${track.id}`}>
      <section className="music-detail-hero">
        <div className="music-detail-hero-image" style={{ backgroundImage: `url(${heroImage})` }} />
        ...
        <div className="music-detail-hero-copy">
          {track.showReleaseLabel !== false && <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.12, ease: EASE }} className="music-detail-release-label">
            {track.releaseLabel || "New release"} <span>{track.releaseDate}</span>
          </motion.p>}
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.18, ease: EASE }}>
            {track.title}
          </motion.h1>
          {track.showArtist !== false && <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3, ease: EASE }} className="music-detail-artist">
            <span /> {artistName}
          </motion.p>}
        </div>
      ...
      {showStorySection && <section className={`music-detail-story ${track.showReleaseNote === false || track.showListenPanel === false ? "is-single" : ""}`}>
        {track.showReleaseNote !== false && <article className="music-detail-copy">
          <span className="music-detail-section-index">01 / RELEASE NOTE</span>
          <h2>{track.title}</h2>
          {track.showReleaseLead !== false && track.description && <p className="music-detail-lead">{track.description}</p>}
          {track.showReleaseStory !== false && track.story && <p>{track.story}</p>}
          {track.showReleaseNoteOutro !== false && <p>{track.releaseNoteOutro || "Made for the spaces between the club, the studio, and the road, the record carries the same intent in every format: movement first, detail second, and a lasting atmosphere after the sound fades."}</p>}
          {track.showCredits !== false && <div className="music-detail-credits">
            <span>Written & produced by</span><strong>{track.writtenBy || artistName}</strong>
            <span>Release year</span><strong>{track.year}</strong>
            <span>Format</span><strong>{track.genre}</strong>
          </div>}
        </article>}

        {track.showListenPanel !== false && <aside className="music-detail-listen">
      ...
      {track.showImmersiveSection !== false && <section className="music-detail-video">
        <div className="music-detail-video-media" style={{ backgroundImage: `url(${heroImage})` }}>
        ...
        <div className="music-detail-video-copy">
          <span>{track.immersiveEyebrow || "02 / IMMERSIVE SOUND"}</span>
          <h2>{track.immersiveTitle || "Listen to the world behind the record."}</h2>
          <p>{track.immersiveDescription || `A visual fragment for ${track.title}, shaped around its pace, colour, and afterimage.`}</p>
        </div>
      </section>}

      {track.showStreamSection !== false && <section className="music-detail-stream">
