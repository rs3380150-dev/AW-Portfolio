  ...
  useEffect(() => {
    ...
    const onPointerMove = (event) => {
      ...
      if (!holdingRef.current && hintRef.current) {
        hintRef.current.classList.toggle("is-visible", !isInteractive(event.target));
      }
    };

    const onPointerDown = (event) => {
      const canvasReady = canvasRef.current?.classList.contains("is-ready");
      if (!finePointer || !canvasReady || event.button !== 0 || isInteractive(event.target) || images.length < 2) return;
      positionHint(event);
